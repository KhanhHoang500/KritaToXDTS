"""Utility Functions for Animation Layers Export

General-purpose helper functions used throughout the animation layers exporter.
"""
...
def compute_content_hash(data: bytes) -> str:
    """Compute an MD5 hash of binary data.
    
    Used for detecting duplicate frame content to avoid redundant exports.
    
    Args:
        data: Binary data (e.g., pixel data from a layer).
...
def make_unique_name(name: str, used_names: set) -> str:
    ...
    Args:
        ...
        used_names: Set of names already in use (will be modified).
        
    Returns:
        A unique version of the name (original if not a duplicate).
        
    Example:
        >>> used = set()
        >>> make_unique_name("Layer", used)
        'Layer'
        >>> make_unique_name("Layer", used)
        'Layer_1'
        >>> make_unique_name("Layer", used)
