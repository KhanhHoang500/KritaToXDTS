"""Layer Operations for XDTS Export

Functions for working with Krita layers during animation export.
"""

from ..config import (
    ANIMATED_LAYER_TYPES,
    LIGHT_TABLE_PREFIX,
    LIGHT_TABLE_NAME,
    REFERENCE_LAYER_COLOR,
)
...
def get_layer_keyframes(layer, start_frame: int, end_frame: int) -> list:
    ...
    # For regular layers, check directly
    keyframes = []
    for frame in range(start_frame, end_frame + 1):
...
def count_total_keyframes(layers: list, start_frame: int, end_frame: int) -> int:
    """Count total keyframes across multiple layers.
    
    Useful for progress bar calculations.
    
    Args:
        layers: List of animated layers.
        start_frame: First frame of range.
        end_frame: Last frame of range.
...
def is_stop_frame(layer) -> bool:
    """Check if a keyframe is a stop frame (blank keyframe).
    
    In Krita, when a user inserts a blank keyframe without adding any pixel data,
    it acts as a "stop frame" that ends the hold of the previous keyframe's content.
    This is detected by checking if the layer has zero-size bounds at that frame,
    indicating no pixel content exists.
    
    The document must be set to the target frame before calling this function.
    
    Args:
        layer: The Krita layer node (document must already be set to the frame).
        
    Returns:
        True if this is a stop frame (blank keyframe with no content).
    """
    bounds = layer.bounds()
